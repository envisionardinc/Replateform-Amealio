...
- **Last updated:** 2026-09-02 (Post-audit reconciliation checkpoint — read-only; no implementation)
...
- **Post-audit reconciliation (checkpoint, read-only):** reconciled completed work (through P1.7.1E) against the independent forensic current-state audit (`cursor/current-state-forensic-audit-07fc`, PR #21; `docs/current-state/`, inspected not merged). Confirmed most prior discovery valid; flagged 3 material corrections — (1) the AI/RAG repo is now known (`amealio-homepage-v2-rag-server`; earlier "external/UNKNOWN") and must **not** be rewritten; (2) target seating model is incomplete vs the real `Diner` lifecycle (missing `INITIAL`, cross-links, ETA/`shareLink`, cron table-status sync vs embedded `subscription.table_setup`); (3) Experiences/Celebrations, Events/Tickets, discovery content, and thin `Subscription` config have **no target design yet**. Output: [current-state/POST-AUDIT-RECONCILIATION.md](../current-state/POST-AUDIT-RECONCILIATION.md) (capability parity, dependency/preservation matrices, per-domain constraints, open owner decisions, recommended sequencing). **Recommended next slice: P1.7.1F — staff/admin RBAC enforcement** on existing `Role`/`RolePermission`. No implementation/schema/migration/frontend changes.

## Current activity

- **P1.7.1E complete.** Staff/admin password authentication works and is validated (local/dev, flagged). **Overall authentication migration is NOT complete** — **RBAC/permission enforcement (P1.7.1F)**, act-as-merchant, staff registration/OTP/social/MFA/reset, and cutover remain pending; **O1** (login-identifier uniqueness) and AUTH-D4/D6/D7 remain owner decisions. **No domain migration started.** Merchant/Location still awaits AUTH-D2 confirmation; OD-11 and owner-decision domains remain blocked.
