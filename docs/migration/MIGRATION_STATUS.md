...
- **Last updated:** 2026-09-02 (P1.7.1D — Staff/Admin Identity PostgreSQL Schema COMPLETE — schema only; authentication deferred)
...
- **P1.7.1D complete (schema only).** The staff/admin identity tables now exist; **staff/admin authentication has NOT started.** Next (pending AUTH-D4/D6/D7 owner decisions and, for login-identifier uniqueness, **O1**): staff credential/session repositories → staff credential + session services → staff login → refresh/logout → JWT guard/principal → merchant scope → RBAC → SUPER_ADMIN → act-as (per doc 26 §18). **Overall authentication migration is NOT complete.** Merchant/Location still awaits AUTH-D2 confirmation; OD-11 and owner-decision domains remain blocked.
