...
| [domains/32-NEXT-SLICE-DEPENDENCY-RECONCILIATION.md](./domains/32-NEXT-SLICE-DEPENDENCY-RECONCILIATION.md) | P1.7.5: **Next-slice dependency reconciliation (discovery only)** — backward dependency analysis recommending **Menu & Catalog** as the next foundation (target schema complete, upstream done, no blocking UNKNOWN); candidate ranking, Menu deep dive, why-not-others, blockers, proposed boundary + test strategy. No code. |
